...
This produces two effects: (1) the orchestrator stays grounded in project-level goals without being consumed by implementation-detail noise, and (2) total token cost is lower than running everyone at 1M. The CPC predecessor measured **22.7% token savings** vs symmetric-max-context across an 11-day production run (128 issues, 175 PRs, 6.8M output tokens + 10.26B cache-read tokens — 1,511:1 cache-read-to-output ratio). See `groundnuty/claude-plan-composer` for the empirical data + `research/2026-03-28-token-usage-empirical-analysis.md` for the breakdown.
